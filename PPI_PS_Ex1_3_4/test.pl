...
($prot_interactions, $protein_seq, $pos) = @ARGV;
...
# Count the sub-sequences
# ***********************
open (File,"<subsequence.txt");
while (@sub=<File>)
{
$sub_no = @sub/2;
}
...
# Gist SVM
#*********
system "./gist-train-svm.exe -train tr_examples.txt -class tr_labels.txt -threshold 1e-10 -radial > traingist.s";
system "./gist-classify.exe -train tr_examples.txt -learned traingist.s -test ts_examples.txt > testgist.s";
system "perl score-svm-results.exe -test ts_labels.txt testgist.s traingist.s";
